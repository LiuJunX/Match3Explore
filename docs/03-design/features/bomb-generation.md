...
1.  **优先级 1：玩家操作位 (Player Input Positions)**
    *   **定义**：玩家刚才交换的两个格子（**起点 Source** 和 **终点 Target**）拥有最高且同等的优先级。
    *   **规则**：
        *   若消除组包含操作格子中的**一个**，炸弹生成在该格子上。
        *   若消除组包含操作格子中的**两个**，从这两个格子中**随机**选择一个。
    *   *场景*：
        *   **常规消除**：玩家拖拽 A 到 B，B 处形成消除 -> 生成在 B。
        *   **异位消除**：玩家拖拽 A 到 B，A 的原位形成消除 -> 生成在 A。
        *   **双位消除**：玩家拖拽 A 到 B，A 和 B 都在同一消除组中 -> 随机选择 A 或 B。
    *   *目的*：确保炸弹始终生成在玩家"手指触碰过"的地方，提供最强的操控反馈。

2.  **优先级 2：随机位置 (Random Position)**
    *   **定义**：在消除组的所有格子中**随机**选择一个。
    *   **场景**：非玩家直接操作导致的消除（如掉落连击、连锁反应）。
    *   *规则*：完全随机，不倾向于中心点或任何特定位置。
    *   *实现*：使用统一随机源 (Random Domain) 确保可重放性。
...
**核心原则**：有结构依据的吸收 — 只有当散落棋子与现有形状存在结构性关联时才吸收。
...
| :--- | :--- | :--- |
| **Simple3** | 无 | 不吸收 |
| **Line4** | Rocket | 共线 + 连续相邻 |
| **Line5** | Rainbow | 共线 + 连续相邻 |
| **Square (2x2)** | UFO | 正交相邻（递归吸收） |
| **Cross (T/L/+)** | TNT/UFO | 与交叉点共线 + 连续相邻 |

##### 规则详解

**1. Simple3 - 不吸收**

普通3连直线不生成炸弹，也不吸收任何散落棋子。
...
**2. Line4/Line5 - 共线 + 连续相邻**

只吸收与直线共线且连续相邻的散落棋子（延长线方向）。
...
A A A A _ S  ✗ S共线但不连续（中间有空格），不吸收
```

**3. Square (2x2) - 正交相邻 + 递归**

吸收与2x2方块正交相邻（上下左右）的散落棋子，并递归吸收与已吸收部分正交相邻的棋子。
...
A A
    ...
    S2
    S1
  A A      ✓ S1正交相邻于2x2，吸收；S2正交相邻于S1，递归吸收
  A A
```

注：在连通分量中，所有棋子都是正交连通的。递归吸收会将所有正交链上的棋子都吸收到2x2中。

**4. Cross (T/L/+) - 与交叉点共线 + 连续相邻**

吸收与交叉点（两条线的相交格子）共线且连续相邻的散落棋子。
等价表述：散落棋子S与交叉点形成的矩形内，每个格子都必须有同色棋子。

*T形示例（交叉点在水平线与垂直线相交处）：*
```text
S A A A    ✓ S与交叉点水平共线且连续，吸收
    A
    A
...
| 形状 | 条件 | 生成物 |
| :--- | :--- | :--- |
| **Plus (+)** | 中心 + 四臂各1格（共5格对称十字） | UFO |
| **T/L形** | 两条≥3的线相交（非对称） | TNT |

**重要**：T形要求水平和垂直两条线都必须≥3个格子。
```text
A A A
  A        这不是T形！垂直只有2格（含交叉点）
           → 按 Simple3 处理，底部A不吸收
...
```
1. 确定最优形状组合（通过 PartitionSolver）
2. 标记所有形状占用的格子
3. 识别散落棋子（未被任何形状占用的格子）
4. 对每个散落棋子：
   a. 找到相邻的形状（通过 BFS）
   b. 根据形状类型判断是否可吸收
   c. 如果可吸收，将散落棋子加入该形状
5. 剩余无法吸收的散落棋子：
   - 如果形成≥3的有效直线，作为独立 Simple3 组处理
   - 否则不参与本次消除（会下落并可能形成新匹配）
```
