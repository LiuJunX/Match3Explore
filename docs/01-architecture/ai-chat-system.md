...
触发深度思考模式（DeepSeek R1）。

| 工具 | 参数 | 输出 |
|:-----|:-----|:-----|
| `need_deep_thinking` | reason, task_summary | 触发 R1 模型，进入 editOnlyMode |

**深度思考流程**：
1. Chat 模型判断任务复杂度，调用 `need_deep_thinking`
2. 切换到 ReasonerModel (R1) 生成设计方案
3. 切换回 Chat 模型，启用 `editOnlyMode` 执行编辑工具
4. `editOnlyMode` 防止在执行阶段调用分析工具，避免无限循环
